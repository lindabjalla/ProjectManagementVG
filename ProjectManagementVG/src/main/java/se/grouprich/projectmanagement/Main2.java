...
{
	public static void main(String[] args) throws TeamException, WorkItemException, RepositoryException, ParseException
	{
		try (AnnotationConfigApplicationContext context = new AnnotationConfigApplicationContext())
		{
			...
			issueService.createAndAddToWorkItem(createdWorkItem.setStatus(WorkItemStatus.DONE), new Issue("Katten sover"));
			...
			while (allIssues.hasNext())
			{
				...
				allIssues = issueService.findAll(allIssues.nextPageable());
			}
			System.out.println("---issues---");
			allIssues.forEach(System.out::println);
			
			Page<WorkItem> allWorkItems = workItemService.findAll(new PageRequest(0, 2));
			while (allWorkItems.hasNext())
			{
				System.out.println("---work items---");
				allWorkItems.forEach(System.out::println);
				allWorkItems = workItemService.findAll(allWorkItems.nextPageable());
			}
			System.out.println("---work items---");
			allWorkItems.forEach(System.out::println);
			
			Page<User> allUsers = userService.findAll(new PageRequest(0, 2));
			while (allUsers.hasNext())
			{
				System.out.println("---users---");
				allUsers.forEach(System.out::println);
				allUsers = userService.findAll(allUsers.nextPageable());
			}
			System.out.println("---users---");
			allUsers.forEach(System.out::println);
			System.out.println();
			
			workItemService.changeWorkItemStatus(workItem2, WorkItemStatus.DONE);
			WorkItem workItemFound = workItemService.findById(20L);
			workItemService.changeWorkItemStatus(workItemFound, WorkItemStatus.DONE);
